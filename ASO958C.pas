...
begin
   ...
   Begin
      xSQL := 'SELECT ASODNI FROM APO201 WHERE ASODNI = '+QuotedStr(DM1.cdsAso.FieldByname('ASODNI').AsString);
      DM1.cdsQry.Close;
   ...
   Begin
      xSQL := 'SELECT ASODNI FROM APO201 WHERE ASODOCNUM = '+QuotedStr(DM1.cdsAso.FieldByname('ASODOCNUM').AsString);
      DM1.cdsQry.Close;
      DM1.cdsQry.DataRequest(xSQL);
      DM1.cdsQry.Open;
      if DM1.cdsQry.RecordCount>0 then
      begin
         ShowMessage('Existe Asociado con CE '+DM1.cdsAso.FieldByname('ASODOCNUM').AsString+'. Verificar...');
         Exit;
      end;
   End;
   ...
   xSQL:='Insert into APO201( '
        +   'ASOID, ASOCODMOD, ASOCODAUX, ASOCODPAGO, ASOAPEPAT, ASOAPEMAT, ASONOMBRES,ASOAPECASDNI, '
        ...
        +   'MDFFECMOD, EMICARTA, DESMANVIV, CODMOTDESTCUO, FECACTPAD, CODPENSION, CODLEYPEN, CODTIPTEL1, CODREFTEL1, CODTIPTEL2, CODREFTEL2, FECREGAUTDESAPO, CENEDUID, D_COD_TUR, FECCADCAREXT)'
        +' Select '
        +   'ASOID, ASOCODMOD, ASOCODAUX, ASOCODPAGO, ASOAPEPAT, ASOAPEMAT, ASONOMBRES,ASOAPECASDNI, '
        +   'ASOAPENOM, USEID, UPROID, UPAGOID, ASORESNOM, ASOFRESNOM, RESID, REGPENID, '
        +   'ASOTIPID, ASOFACT, CASE WHEN ASOTIPID = ''VO'' THEN ''DO'' ELSE ASOTIPID END,'
   ...
   xSql := 'SELECT R.DIRECCION, R.LUGDOM FROM SIOREN.SIO_CONSULTA_IDENTIDAD R WHERE R.DNI = ' + QuotedStr(DM1.cdsAso.FieldByName('ASODNI').AsString);   
   DM1.cdsQry.Close;
   DM1.cdsQry.DataRequest(xSql);
   DM1.cdsQry.Open;
   if DM1.cdsQry.RecordCount > 0 then
     ...
     begin
       ...
       begin
         xSql := 'SELECT TRUNC(R.FECACT) FECACT, R.DIRECCION, R.LUGDOM, SUBSTR(R.LUGDOM, 1, INSTR(R.LUGDOM, '','', 1, 1) - 1) DPTO,'
           ...
           + ' FROM SIOREN.SIO_CONSULTA_IDENTIDAD R LEFT JOIN TGE158 D ON SUBSTR(R.LUGDOM, 1, INSTR(R.LUGDOM, '','', 1, 1) - 1) = D.DPTODES'
