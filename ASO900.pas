...
uses  ASODM , ASO908, ASO944, ASO948, COB903, ASO260, ASO901;   //RMZ ASO901,
//  DateUtils;
...
//Se adicionó campos de contrato digital
          +'CORREO_CONTACTO_DIGITAL,FUENTE_CORREO_CONT_DIGITAL,CELULAR_CONTACTO_DIGITAL,FUENTE_CELULAR_CONT_DIGITAL ';
//Fin MAS_202006_HPC
...
If Key = #13 then
     Begin
       ...
       DM1.cdsAso.Close;
       DM1.cdsAso.DataRequest('Select '+xSelect+' From APO201 Where ASOCODMOD LIKE '+QuotedStr(Trim(EdtCodMod.Text)+'%'));
       DM1.cdsAso.Open;
...
begin
 ...
 If ValidaActualizacion()='N' Then
  ...
  DM1.cdsTransCre.DataRequest('SELECT FORPAGOID,FORPAGODES FROM COB101 WHERE FLGDCT=''S'' AND ACTIVO=''S'' ');
  DM1.cdsTransCre.Open;
  DM1.cdsTipDesem.Close;
  DM1.cdsTipDesem.DataRequest('SELECT TIPDESEID,TIPDESEABR FROM CRE104  WHERE FLGLISTA=''S'' AND ACTIVO=''S'' ORDER BY TIPDESEID');
...
begin
xAsoId:=Trim(DM1.cdsAso.FieldByName('ASOID').AsString);
xSQL:='SELECT asoid, asoapenom, mondepo, asodni, asodni_ser, credid, crefotorg, crefdep, docdepo,'
     +'       desefe, usrcre, usrcaj, fotocre, fregcaj, asocodmod, usrasig, estdep, depdev, nrotran '
     +'FROM SRB000 WHERE ASOID='+QuotedStr(xAsoId)+' AND NROTRAN IS NOT NULL AND NVL(DEPDEV,0)=0';
DM1.cdsAbonoCreditos.Close;
DM1.cdsAbonoCreditos.DataRequest(xSQL);
DM1.cdsAbonoCreditos.Open;
If DM1.cdsAbonoCreditos.RecordCount > 0 Then
