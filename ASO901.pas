...
// MAS_202309_HPC         :  En qry se añade la columna DPTOID
...
uses ASODM, ASO908, ASO936, ASO900, ASO260, ASO942; //RMZ  ASO942,  ASO903,
...
//Inicio MAS_202105_HPC 
          +'CORREO_CONTACTO_DIGITAL,FUENTE_CORREO_CONT_DIGITAL,CELULAR_CONTACTO_DIGITAL,FUENTE_CELULAR_CONT_DIGITAL,AUTDESAPOFORMATO ';
//Fin MAS_202105_HPC 
//Fin MAS_202006_HPC
  ...
  If Length(Trim(edtBuscar.Text)) > 0 Then
     Begin
       ...
       If rbtApeNom.Checked Then xSQL:='SELECT '+xSelect+' FROM APO201 WHERE ASOAPENOMDNI LIKE '+QuotedStr(Trim(edtBuscar.Text)+'%');

       If rbtDni.Checked Then xSQL:='SELECT '+xSelect+' FROM APO201 WHERE ASODNI LIKE '+QuotedStr(Trim(edtBuscar.Text)+'%');

       If rbtCuenta.Checked Then xSQL:='SELECT '+xSelect+' FROM APO201 WHERE ASONCTA LIKE '+QuotedStr(Trim(edtBuscar.Text)+'%');

       If rbtCodMod.Checked Then xSQL:='SELECT '+xSelect+' FROM APO201 WHERE ASOCODMOD LIKE '+QuotedStr(Trim(edtBuscar.Text)+'%');
...
Var xAsoId,xSQL,xSelect:String;
Begin
  xSelect:='asoid, asocodmod, asocodaux, asocodpago, asoapepat, asoapemat, asonombres, asoapenom, useid, uproid, upagoid,'
          +'asoresnom, asofresnom, resid, regpenid, asositid, asofact, asotipid, viaid, vianombre, asonumdir, asofdpto,'
          +'asodpto, zonaid, zonanomb, ubigeoid, dptoid, paisid, refdom, asofecnac, asotelf1, asotelf2, tipdoccod, asodocnum,'
  ...
  Begin
    ...
    Begin
      Try
        ...
        xSQL:='SELECT '+xSelect+' FROM APO201 WHERE ASOID='+QuotedStr(Trim(xAsoId));
        DM1.cdsAso.Close;
        DM1.cdsAso.DataRequest(xSQL);
        DM1.cdsAso.Open;
        ...
        Begin
          ...
          xSQL:= 'SELECT asoid, usuario, hreg, idcodigo, asoncta, asoapenom, asodni, fecaut, usraut, motaut, fhau FROM ASO002 WHERE ASOID='''+xAsoId+''' ';
          DM1.cdsCtasBco.Close;
          DM1.cdsCtasBco.DataRequest(XSQL);
          DM1.cdsCtasBco.Open;
...
begin
  xSelect:='asoid, asocodmod, asocodaux, asocodpago, asoapepat, asoapemat, asonombres, asoapenom, useid, uproid, upagoid,'
          ...
          +'codreglab, usuautonp, oriopeonp, codciutelfij1, codciutelfij2, fecreccadaps, usureccadaps, codmotautcadaps, codpre, asocci,'
//Inicio MAS_202105_HPC 
          +'latitud_dir, longitud_dir,AUTDESAPOFORMATO  ';
//Fin MAS_202105_HPC 
    ...
    XSQL:='SELECT '+xSelect+' FROM APO201 WHERE ASOID=''AAAAAAAAAA'' ';
...
//Fin MAS_202006_HPC
  ...
  fSeleccion.edtBuscar.Text:=''; DM1.cdsAso.Close;  DM1.cdsAso.DataRequest('SELECT '+xselect+' FROM APO201 WHERE ASOCODMOD='+QuotedStr('@#$!*@/#&^'));   DM1.cdsAso.Open;
end;
