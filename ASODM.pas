...
// MAS_201905_HPC            : Se crea variable publica para guardar codido de area 
// HPC_201906_MAS          : Resoluciones grabar historial
// MAS_201907_HPC          : Se añade variable global para controlar la dec/aut de descuento de aportes
// MAS_201909_HPC          : se añaden variables para el control captura de documentos de cancelación.
// MAS_202006_HPC          :  botón para visualizar datos de contrato digital
// MAS_202104_HPC          : Cambio en la estructura de regimen de pensión para la cobranza
// MAS_202105_HPC          : Se adiciona formato de Autorización de descuento
// MAS_202204_HPC          : Se valida que codigo modular no sea repetido
// MAS_202303_HPC          :
// MAS_202309_HPC          : Modificación en el reporte "AUTORIZACIÓN DE DESCUENTO MEDIANTE LA PLANILLA ÚNICA DE PAGO"  
// MAS_202403_HPC          : Cambios según Memo 086-DM/PS-2022, 134-DM/PS-2023, 059-DM/PS-2023
...
uses
   ...
   Excel2000, OleServer,DateUtils, jpeg, ExcelXP,
   ...
   ppSubRpt, IdHTTP, uLkJSON,
   fcButton, fcImgBtn, fcShapeBtn, ExtCtrls, StdCtrls, Buttons,
   wwdblook, wwdbdatetimepicker, Mask,  ComCtrls, InvokeRegistry, Rio,
   SOAPHTTPClient, xmldom, types,  Grids, ValEdit,
   Wwdbigrd,  fcLabel,
   ppDB,
   ppDBPipe, ppDBBDE, ppBarCod, IdBaseComponent, IdComponent,
   IdTCPConnection, IdTCPClient, IdFTP,
   ppEndUsr,  Wwdbdlg,
   WSUbicabilidad, XSBuiltIns,  LinarBitmap, ExtDlgs, FileUtils, SIPSvc, SIPSvcUTIL, ICOLoader, JPEGLoader, PNGLoader, HIPSLoader, BMPLoader, PCXLoader, WMFLoader, ppVar,
   rxgif, Animate, GIFCtrl;
...
type
  ...
  TDM1 = class(TDataModule)
    ...
    cdsTipCon: TwwClientDataSet;
    dsTipCon: TwwDataSource;
    cdsMotModCon: TwwClientDataSet;
    dsMotModCon: TwwDataSource;
// Inicio  HPC_201906_MAS          : Resoluciones grabar historial
    cdsResolNom: TwwClientDataSet;
    dsResolNom: TwwDataSource;
    cdsResolCon: TwwClientDataSet;
    dsResolCon: TwwDataSource;
    ppletotipdoc: TppLabel;
// Fin   HPC_201906_MAS          : Resoluciones grabar historial
    ...
    function Validacodmodrepetido(xasocodmod, xasoid: String): boolean;
  ...
  anoinicial, mesinicial, diainicial: Word;
   ...
   cIP, wOfiId, widepc, ideconex, fechorcon :String;
    ...
    Jpg,jpgFirma, JpgProDat01, JpgProDat02: TJpegImage;
    gif,gifFirma: TGIFImage;
    Stream,StreamFirma, StrProDat01, StrProDat02: TMemoryStream;
    xNumero, xArea,xCnd,xSal,xGrabaExc,xSgr,xOfiDes,xInsert,xFto,xFma,xDNI,xautdes:String;
    xFlaIma, xNomIma:String;
    xDecAut:String;
    xsindata:integer;
    ...
    xCtlCon:String;
    xDIA1,xMES1,xANO1,xDIREC1,xDPTO1,xREF1,xTLF1,xCEL1,xEMAIL1,xCOLE1,xANONOM1,xRESNOM1:string;
    jpgFoto: TJpegImage;
    dFundacionDerrama : TDate ;
    sUrlGETCONSENTIMIENTO: String;
    sHost: String;
    sPort: Integer;
   ...
   Function ValidaAutoCtas(xAsoid: String): Boolean;
    ...
    Procedure InsertaAso();
    Procedure ModificaAso();
    Procedure AutorizaAso();
    Procedure DesAutorizaAso();
    Procedure NuevoAso_Modifica();
    Procedure Cargacampos();
    Function ConsultaReniec(xAsodni:String; xCodOpcion:String; xUsuReg: String; xIrReniec: Boolean): TlkJSONobject;    
    Function get_DatosReniecLocal(xAsodni:String): TlkJSONobject;
    Procedure CargaRENIEC(xDni, xcodopcion: String);
    procedure CargaImagenesDNI(wsDNI, wsOrigen: String);
    Function EnviarSMS(xCelular:String;xMensaje:String):String;
  end;
var
  DM1: TDM1;
  sSQL, xBorrar : String;
...
uses MsgDlgs, ASO260,  ASO262,  ASO900, COB903, ASO948;
...
//Inicio MAS_202104_HPC
            xSQL:='SELECT REGPENID,REGPENDES,REGPEN FROM APO105 ORDER BY REGPENID';
//Fin MAS_202104_HPC
            ...
            // Se añade columna DPTOID
            // xSQL:='SELECT UPROID,UPAGOID,USEID,USENOM FROM APO101 WHERE NVL(FDESHABILITADO,''S'') =''N''';
            xSQL:='SELECT UPROID, UPAGOID, USEID, USENOM, DPTOID FROM APO101 WHERE NVL(FDESHABILITADO,''S'') =''N''';
            // Fin: MAS_202303_HPC
...
var
 xWhere, wSQL,XSQL :string;
 inifile : TIniFile;
begin

   DCOM1.Connected:=False;
   XUSUARIO :=TRIM(ParamStr(1));
   XPASS    :=TRIM(ParamStr(2));
   XASOID   :=TRIM(ParamStr(3));
   XINSERTA :=TRIM(ParamStr(4));
   XCODPALI_ORIGEN :=TRIM(ParamStr(5));
  ...
  try
     ...
     ShowMessage( 'ERROR de Conexión. 2');
     Exit;
  end;
    ...
    // ******************************************************************
    // COMENTAR ANTES DE REALIZAR EL PASE A PRODUCCION
    // 10 = MODIFICA NUEVO ASOCIADO
    // 11 = INSERTA NUEVO ASOCIADO
    // 0 = MODIFICA ASOCIADO
  ...
  DM1.DCOM1.AppServer.Conectate('xxxxxx',XUSUARIO, XPASS, cAccesoSistema);
  wUsuario := XUSUARIO;
  xArea:=Copy(CrgArea(wUsuario),1,3); xOfiDes:=Copy(CrgArea(wUsuario),4,2);
  ...
  begin
     ...
     xsql := 'SELECT campo, descripcion, flag, objeto, tiempo, tieale, valper, valrec, grupo FROM CAMPOS_VALIDA';
   ...
   // xSql := 'SELECT ASOTIPID, ASOTIPDES FROM APO107 WHERE ASOTIPID IN (''DO'',''CE'', ''CO'')';
      xSql := 'SELECT ASOTIPID, ASOTIPDES FROM APO107 WHERE ASOTIPID IN (''VO'',''CE'', ''CO'')';
      DM1.cdsTAso.Close;
      ...
      xSQL := 'SELECT UPROID,UPAGOID,USEID,USENOM,DPTOID FROM APO101 ';
      DM1.cdsUSE.Close;
...
//Fin MAS_202104_HPC
      ...
      DM1.cdscarta.Close; DM1.cdscarta.DataRequest(xSQL);  DM1.cdscarta.Open;
//Inicio MAS_202303_HPC
      xSql:='SELECT DESHOST,DESPORT,UrlGETCONSENTIMIENTO FROM DB2ADMIN.ASO_PAR_GEN_HOST WHERE NVL(ACTIVO,''N'')=''S'' ';
      DM1.cdsQry.Close;
      DM1.cdsQry.DataRequest(xSql);
      DM1.cdsQry.Open;
      If DM1.cdsQry.RecordCount>0 Then
      Begin
       sHost:= DM1.cdsQry.Fieldbyname('DESHOST').AsString;
       sPort:= DM1.cdsQry.Fieldbyname('DESPORT').AsInteger;
       sUrlGETCONSENTIMIENTO:=DM1.cdsQry.Fieldbyname('UrlGETCONSENTIMIENTO').AsString;
      End
      Else
      Begin
       sHost:= 'dmsmtp.derrama.org.pe';
       sPort:= 25;
       sUrlGETCONSENTIMIENTO:='http://dmgeneral.derrama.org.pe/lpdp/get_estado_lpdp';
      End;
      If (XINSERTA='2') or (XINSERTA='10') or (XINSERTA='11') or (XINSERTA='3')Then
      Begin
         ...
         +' EMICARTA, DESMANVIV, CODMOTDESTCUO, FECACTPAD, EST_AUT_CLI, asoapecasdni,codleypen,codpension,codpre,'
         +' USU_AUT_CLI, FEC_AUT_CLI, OBS_AUT_CLI, USU_CRE_CLI, FEC_CRE_CLI,CODREFTEL1,CODREFTEL2,codtiptel1, codreftel1, codtiptel2, codreftel2,'
         +' OBS_AUT_CLI_UGE, OBS_AUT_CLI_PAD, OBS_AUT_CLI_OTR, CENEDUID,codciutelfij1, codciutelfij2,'
         +' FALLECIDO, TIPO_FALL_ID,codblocharen, numblocharen, coddepintren, desdepintren, nvl(validaaporte,''S'') validaaporte,'
         +' null CORREO_CONTACTO_DIGITAL, null FUENTE_CORREO_CONT_DIGITAL, null CELULAR_CONTACTO_DIGITAL, null FUENTE_CELULAR_CONT_DIGITAL, D_COD_TUR,  NVL(AUTDESAPO,''N'') DES_APO,'
         +' AUTDESAPOFORMATO, FECCADCAREXT'
         +' from  DB2ADMIN.ASO_NUE_CLI'
      ...
      Begin
         ...
         ' CORREO_CONTACTO_DIGITAL, FUENTE_CORREO_CONT_DIGITAL, CELULAR_CONTACTO_DIGITAL, FUENTE_CELULAR_CONT_DIGITAL, AUTDESAPOFORMATO, D_COD_TUR, NVL(AUTDESAPO,''N'') DES_APO,'+
         ' FECCADCAREXT'+
         ' FROM DB2ADMIN.APO201 WHERE ASOID = '+QuotedStr(XASOID);
      end;
      DM1.cdsAso.Close;
     ...
     try
       ...
       Begin
         If (Length(trim(XINSERTA))= 0) or (trim(XINSERTA)='0') Then
         begin
            ModificaAso;
         end
         Else if trim(XINSERTA)='11' Then
         begin
            InsertaAso;
         End
         Else if trim(XINSERTA)='10' Then
         begin
            NuevoAso_Modifica;
         End
         Else if trim(XINSERTA)='2' Then
         Begin
            AutorizaAso;
         End
         Else if trim(XINSERTA)='3' Then
         Begin
             DesAutorizaAso;
         End;
         Cargacampos;
...
begin
  ...
  sSQL := 'SELECT ciaid, ciades, ciaabr, ciauser, ciafreg, ciahreg, ciactarei, ciatdiarei, ciaacprc, ciaruc, ciadire, '
         +'ciacprc, ciatlf, ciafax, ciatasa, tdaids, tdaide, triids, triide, ciactacot, cialugar, ciasugoc, prorateo, '
         +'ciaanexo, ciasugrq, igvsn, rutarep, cjaautonom, cntconsol, ciaretigv, cntsolodolar, anioglosa, flag, exoigv,'
         +'notingaut, mulforpag, flgserv, ubigeoid '
         +'FROM TGE101 ORDER BY CIAID';
...
var xSQL : String ;
begin
   xSQL :='SELECT tmonid, fecha, tcamvbc, tcamvbv, tcamvpc, tcamvpv, tcamvoc, tcamvov, tcamvxc, tcamvxv FROM TGE107 '+
...
begin
   cdsUser.Close;
   cdsUser.DataRequest('SELECT U.USERID,U.USERNOM,U.AREA,U.ORIGEN,U.NIVEL,NIV.USERNIV, U.CARGO'
   ...
   If cdsUser.RecordCount=1 Then
      Result:=cdsUser.fieldByname('AREA').AsString // 1,3
             ...
             +cdsUser.fieldByname('CARGO').AsString // 10,30
   Else
      Result:='';
   cdsUser.Close
end;
...
begin
  ...
  xSql := 'INSERT INTO APO201_LOG(ASOID, ASOCODMOD, ASOCODAUX, ASOCODPAGO, ASOAPEPAT, ASOAPEMAT, ASONOMBRES, ASOAPENOM, USEID, UPROID, UPAGOID, ASORESNOM, ASOFRESNOM, RESID, REGPENID, ASOSITID,  ASOFACT,  ASOTIPID,'
         ...
         +'       CODTIPTEL1, CODREFTEL1, CODTIPTEL2, CODREFTEL2, CODPENSION, D_COD_TUR)'
...
var
   ...
   PictureFoto: TPicture;
   PictureFirma: TPicture;
begin
   If DM1.cdsAso.FieldByName('TIPDOCCOD').AsString = '10' Then
   Begin
      DM1.CargaRENIEC(wsDNI, '000437');
      If DM1.Reg = Nil Then Exit;
      If wsOrigen='ASOCIADO' Then
      Begin
         If(DM1.Reg.Field['FOTO'].Value <> '') Then
         Begin
            Try
               ShowImageFromBase64(DM1.Reg.Field['FOTO'].Value, FMantAsociado.ImagAso);
         	  Except
            End;
         End;
         If(DM1.Reg.Field['FIRMA'].Value <> '') Then
         Begin
            Try
               ShowImageFromBase64(DM1.Reg.Field['FIRMAJPG'].Value, FMantAsociado.ImaFirma);
            Except
            End;
         End;
      End;
      ...
      Begin
         ...
         Begin
            Try
               ShowImageFromBase64(DM1.Reg.Field['FOTO'].Value, FbusasorecCADAPS.ImagAso);
         	 Except
            End;
         End;
         If(DM1.Reg.Field['FIRMA'].Value <> '') Then
         Begin
            Try
               ShowImageFromBase64(DM1.Reg.Field['FIRMAJPG'].Value, FbusasorecCADAPS.ImaFirma);
            Except
            End;
         End;
      End;
      ...
      Begin
         ...
         Begin
            Try
               ShowImageFromBase64(DM1.Reg.Field['FOTO'].Value, fCronoPag.imgFoto);
               PictureFoto := TPicture.Create;
               PictureFoto.Graphic := fCronoPag.imgFoto.Picture.Graphic;
               fCronoPag.ppImage1.Picture.Assign(PictureFoto);
         	  Except
            End;
         End;
      End;
      If wsOrigen='ppRReniec' Then
      Begin
         If(DM1.Reg.Field['FOTO'].Value <> '') Then
         Begin
            Try
               ShowImageFromBase64(DM1.Reg.Field['FOTO'].Value, FMantAsociado.imgFoto);
               PictureFoto := TPicture.Create;
               PictureFoto.Graphic := FMantAsociado.imgFoto.Picture.Graphic;
               FMantAsociado.ppFoto.Picture.Assign(PictureFoto);
         	  Except
            End;
         End;
         If(DM1.Reg.Field['FIRMA'].Value <> '') Then
         Begin
            Try
               ShowImageFromBase64(DM1.Reg.Field['FIRMAJPG'].Value, FMantAsociado.imgFirma);
               PictureFirma := TPicture.Create;
               PictureFirma.Graphic := FMantAsociado.imgFirma.Picture.Graphic;
               FMantAsociado.ppFirma.Picture.Assign(PictureFirma);
         	  Except
            End;
         End;
      End;
      If wsOrigen = 'fIdentAso' Then
      Begin
         If(DM1.Reg.Field['FOTO'].Value <> '') Then
         Begin
            Try
               ShowImageFromBase64(DM1.Reg.Field['FOTO'].Value, fIdentAso.ImagAso);
          	Except
            End;
         End;
         If(DM1.Reg.Field['FIRMA'].Value <> '') Then
         Begin
            Try
               ShowImageFromBase64(DM1.Reg.Field['FIRMAJPG'].Value, fIdentAso.ImaFirma);
            Except
            End;
         End;
      End;
...
xsql:=' Select ''RENIEC'' DATOS,DNI ASODNI,COUNT(1) MOVIMIETOS from SIOREN.SIO_CONSULTA_IDENTIDAD WHERE DNI='''+xAsoDNI+''' GROUP BY DNI ';
...
Xsql:= 'SELECT NVL(ESTDNI,''NO CADUCO'') ESTDNI '
      ...
      +'         END ) ESTDNI '
      +'  From  SIOREN.SIO_CONSULTA_IDENTIDAD I WHERE dni='''+vDNI+''' AND FECEMIDOC IN (SELECT MAX(FECEMIDOC) '
      +'FROM SIOREN.SIO_CONSULTA_IDENTIDAD '
      +'WHERE DNI = '''+vDNI+''')  '
      +')  ';
...
begin
   ...
   +' FROM SIOREN.SIO_CONSULTA_IDENTIDAD WHERE DNI = '+QuotedStr(xAsodni) );
      ...
      Begin
         ...
         Begin
            ...
            Begin
               ...
               DM1.cdsForPag.FieldByName('APECASCOB').AsString := DM1.cdsDetCartas.FieldByName('DSGAPECAS').AsString;
      ...
      Begin
         ...
         Begin
            ...
            Begin
               ...
               DM1.cdsForPag.FieldByName('APECASCOB').AsString := DM1.cdsDFam.FieldByName('FAMAPECAS').AsString;
...
// Para los nombrados en marzo/2024, no se debe visualizar dato laboral, DRE/UGEL y lugar y fecha
procedure TDM1.LlenaDatosPlaUniPag(xasoid: String);
Var xSql :String;
begin
   xSql := 'SELECT A.USUREGAUTDESAPO, A.ASOFRESNOM, A.ARCHIVOFTP, A.ASOAPENOM, A.ASOAPENOMDNI, A.ASODNI, A.TIPDOCCOD, A.ASODOCNUM, B.USENOM FROM '+DM1.sAPO201+' A, APO101 B WHERE A.ASOID = '+QuotedStr(xAsoId)+' AND A.UPROID = B.UPROID AND A.UPAGOID = B.UPAGOID AND A.USEID = B.USEID';
   DM1.cdsQry.Close;
   DM1.cdsQry.DataRequest(xSQL);
   DM1.cdsQry.Open;
   DM1.pplasoapenomdni.Caption := DM1.cdsQry.FieldByName('ASOAPENOMDNI').AsString;

   //DM1.pplasodni.Caption := DM1.cdsQry.FieldByName('ASODNI').AsString;
   If DM1.cdsQry.FieldByName('TIPDOCCOD').AsString = '10' Then
   Begin
      ppletotipdoc01.Caption := 'Identificado con DNI Nº';
      pplnumdocide01.Caption := DM1.cdsQry.FieldByName('ASODNI').AsString;
   End
   Else
   Begin
      ppletotipdoc01.Caption := 'Identificado con CE Nº';
      pplnumdocide01.Caption := DM1.cdsQry.FieldByName('ASODOCNUM').AsString;
   End;

   If (DM1.cdsQry.FieldByName('ARCHIVOFTP').AsString = 'NN2024') And (DM1.cdsQry.FieldByName('ASOFRESNOM').AsString >= '01/03/2024')
   And (DM1.FechaSys < DM1.cdsQry.FieldByName('ASOFRESNOM').AsDateTime) Then
   Begin
      DM1.pplconlabnom.Caption := '(   )';
      DM1.pplUGEL.Caption := '';
      DM1.ppllugarfecha.Caption := '';
   End
   Else
   Begin
      DM1.pplUGEL.Caption := DM1.cdsQry.FieldByName('USENOM').AsString;
...
procedure TDM1.LlenaDatosPlaDecAso(xasoid: String);
Var xsql, xfijo, xcelular:String;
begin
   xsql := 'SELECT A.ASOFRESNOM, A.ASOAPENOMDNI, A.TIPDOCCOD, A.ASODOCNUM, A.ASODNI, B.UPRONOM, C.USENOM, REPLACE(A.ASODIR, ''(Ref.''||Trim(A.DESREFVIV)||'')'',NULL) ASODIR,'
...
//Inicio MAS_202105_HPC
   ...
   If DM1.cdsQry.FieldByName('TIPDOCCOD').AsString = '10' Then
   Begin
      ppletotipdoc.Caption := 'DNI.';
      pplnumdocide.Caption := DM1.cdsQry.FieldByName('ASODNI').AsString;
   End
   Else
   Begin
      ppletotipdoc.Caption := 'CE.';
      pplnumdocide.Caption := DM1.cdsQry.FieldByName('ASODOCNUM').AsString;
   End;

   pplanonom_dec.Caption := Copy(DM1.cdsQry.FieldByName('ASOFRESNOM').Asstring,7,4);
   xANONOM1 := Copy(DM1.cdsQry.FieldByName('ASOFRESNOM').Asstring,7,4);
   xDIA1 := Copy(DM1.FechaHora,1,2);
   xMES1 := LowerCase(DM1.NombreMes(Copy(DM1.FechaHora,4,2)));
   xANO1 := Copy(DM1.FechaHora,7,4);
   ...
   xDIREC1 := DM1.cdsQry.FieldByName('ASODIR').AsString;
   xDPTO1  := DM1.cdsQry.FieldByName('DEPPRODIS').AsString;
   xREF1   := DM1.cdsQry.FieldByName('REFDOM').AsString;
   xEMAIL1 := DM1.cdsQry.FieldByName('ASOEMAIL').AsString;
   xCOLE1  := DM1.cdsQry.FieldByName('NOMCENEDU').AsString;
 //xANONOM1:= DM1.cdsQry.FieldByName('ASOFRESNOM').AsString;
   xRESNOM1:= DM1.cdsQry.FieldByName('ASORESNOM').AsString;
//Fin MAS_202105_HPC
...
function TDM1.VerificaVersion: Boolean;
Var
   xSQL: String;
Begin
   Result := False;
   ...
   If FMantAsociado.lblVersion.Caption = DM1.cdsCEdu.FieldByName('VERSION').AsString Then
      Result := True
   Else
   Begin
      ShowMessage('Para poder continuar, es necesario que actualice la Versión del MANASO');
      DM1.cdsCEdu.Close;
      DM1.DCOM1.Connected := False;
      Result:=False;
      Application.Terminate;
...
//Fin MAS_202104_HPC
      ...
      xSQL:='SELECT DPTOID, UPROID, UPAGOID, USEID, USENOM'
      ...
      If DM1.xCnd = 'AUT' Then
        begin
           ...
           FMantAsociado.btncodpago.Visible     :=False;
           ...
           begin
              ...
              FMantAsociado.btncodpago.Visible     :=True;
          end;
...
procedure TDM1.NuevoAso_Modifica;
var xsql:String;
begin
   DM1.sAPO201:='ASO_NUE_CLI';
   DM1.xInsert:='0';
   ...
   xSQL := 'SELECT DPTOID, UPROID,UPAGOID,USEID,USENOM, DPTOID FROM APO101 WHERE NVL(FDESHABILITADO,''S'') = ''N''';
   DM1.cdsUSE.Close;
   DM1.cdsUSE.DataRequest(xSQL);
   DM1.cdsUSE.Open;

   xSQL := 'SELECT CIUDID AS PROVID, CIUDDES AS PROVDES FROM APO123';
   DM1.cdsHijo.Close;
   ...
   // distrito DNI
   xSQL := 'SELECT ZIPID, ZIPDES FROM APO122';
   DM1.cdsQry7.Close;
...
//Carga datos
procedure TDM1.Cargacampos;
begin
   If Trim(XINSERTA) = '11' Then Exit;
   If DM1.cdsAso.FieldByName('TIPDOCCOD').AsString = '10' Then
      DM1.CargaImagenesDNI(Trim(DM1.cdsAso.FieldByName('ASODNI').AsString),'ASOCIADO');
   ...
   FMantAsociado.lkcDist2.Text := DM1.cdsAso.fieldbyname('ASODSTLABID').AsString;
   FMantAsociado.edtanos.Text := DM1.tiempoTranscurrido(dm1.FechaSys,FMantAsociado.dbdtFecNac.Date);
   FMantAsociado.dblcdCodLeyPen.Text :=  Trim(DM1.cdsAso.fieldbyname('CODLEYPEN').AsString);
   FMantAsociado.edtCodLeyPen.Text :=  DM1.CrgDescrip('CODLEYPEN = '+QuotedStr(Trim(DM1.cdsAso.fieldbyname('CODLEYPEN').AsString)),'ASO_LEY_PEN_MAE','DESLEYPEN');
   FMantAsociado.dblcdCodPrestacion.Text :=  Trim(DM1.cdsAso.fieldbyname('CODPRE').AsString);
   FMantAsociado.edtdesprestacion.Text := DM1.CrgDescrip('CODPRE = '+QuotedStr(Trim(DM1.cdsAso.fieldbyname('CODPRE').AsString)),'ASO_COD_PRE_MAE','DESPRE');
   FMantAsociado.lblAsoApeNom.Caption:=DM1.cdsAso.fieldbyname('ASOAPENOM').AsString+' (Dato de Planillas)';
   FMantAsociado.edtCodPensionista.Text:=DM1.cdsAso.fieldbyname('CODPENSION').AsString;
   If DM1.CrgDescrip('ASOID='+QuotedStr(Trim(xAsoId)),'SRB000','ESTDEP')='02' Then
      FMantAsociado.pnlPrefente.Visible:=True
   Else
     FMantAsociado.pnlPrefente.Visible:=False;
...
// Se valida que el codigo modular que se va a registrar no exista en la base de datos de maestra de asociados.
function TDM1.Validacodmodrepetido(xasocodmod, xasoid: String): boolean;
Var xSql:String;
begin
   // Inicio: MAS_202303_HPC
   // Para la tabla ASO_NUE_CLI, se controla que solo filtre registros que no estan aprobados
   // xSql := ' SELECT ASOID, ASODNI, ASOAPENOMDNI FROM ASO_NUE_CLI WHERE ASOCODMOD = '+QuotedStr(Trim(xasocodmod))
   xSql := ' SELECT ASOID, ASODNI, ASOAPENOMDNI FROM ASO_NUE_CLI WHERE ASOCODMOD = '+QuotedStr(Trim(xasocodmod))+' AND NVL(EST_AUT_CLI,''XX'')  <> ''AP'''
          +' UNION ALL'
          +' SELECT ASOID, ASODNI, ASOAPENOMDNI FROM APO201 WHERE ASOCODMOD = '+QuotedStr(Trim(xasocodmod));
   // Fin: MAS_202303_HPC          
   DM1.cdsQry.Close;
   DM1.cdsQry.DataRequest(xSQL);
   DM1.cdsQry.Open;
   ...
   Begin
      If DM1.cdsQry.RecordCount = 1 Then
      Begin
         MessageDlg(' El código modular : '+Trim(xasocodmod)+' ya se encuentra registrado en el sistema.'+#13+' pertenece al asociado: '+DM1.cdsQry.FieldByName('ASOAPENOMDNI').Asstring, mtInformation, [mbOk], 0);
         result := false;
         Exit;
      End;
   End;
   If (DM1.cdsQry.RecordCount = 1) And (xasoid <> DM1.cdsQry.FieldByName('ASOID').AsString) Then
   Begin
      MessageDlg(' El código modular : '+Trim(xasocodmod)+' ya se encuentra registrado en el sistema.'+#13+' pertenece al asociado: '+DM1.cdsQry.FieldByName('ASOAPENOMDNI').Asstring, mtInformation, [mbOk], 0);
      result := false;
      Exit;
   ...
   If (DM1.cdsQry.RecordCount > 1) Then
   Begin
      MessageDlg('Existe mas de un código modular repetido.', mtInformation, [mbOk], 0);
      result := false;
      Exit;
   End;
   result := true;
...
procedure TDM1.CargaRENIEC(xDni, xcodopcion: String);
begin
   //--Ir al servicio de consulta de Reniec en General--//
   If DM1.CrgDescrip('COD_PAR=' + QuotedStr('01'), 'CRE_PAR_GEN_SIS', 'ACT_RENIEC_MAESTRO')='S' Then
   Begin
      Try
         Reg := DM1.ConsultaReniec(Trim(xDni), xcodopcion, DM1.wUsuario,false);
      Except
         On E: Exception do
         Begin
      ...
      Except
         ...
         End;
      End;
      If ( (Reg.Field['ICONO'].value = 0) and (Reg.Field['MENSAJE'].value <> null) ) Then
      Begin
         MessageDlg(Reg.Field['MENSAJE'].Value, mtError,[mbOk],0);
         Reg := Nil;
         Exit;
...
function TDM1.ConsultaReniec(xAsodni, xCodOpcion, xUsuReg: String;
  xIrReniec: Boolean): TlkJSONobject;
var
 HTTP1: TIdHTTP;
 Json: string;
 sResponse: string;
...
begin
   ...
   try
     ...
     sServidorWeb:=sServidorWeb+'/reniec/buscarenReniec';
     Json:='{'
            +  '"ASODNI":"'+xAsodni+'",'
            +  '"CODAPLI":"'+'043'+'", '
            +  '"CODOPCION":"'+xCodOpcion+'", '
            +  '"PROCESO":1, '
            +  '"CODARECARCONS":22,';
     if xIrReniec then
        Json:= Json +  '"IRRENIEC":true,'
     else
        Json:= Json +  '"IRRENIEC":false,';
     Json:=
     Json   +  '"USUREG":"'+xUsuReg+'", '
            +  '"DISPOSITIVO":"'+'DESKTOP'+'", '
            +  '"OS":"'+'WINDOWS'+'", '
...
Function TDM1.get_DatosReniecLocal(xAsodni:String): TlkJSONobject;
var
 HTTP1: TIdHTTP;
 Json: string;
 sResponse: string;
...
begin
   ...
   try
     sServidorWeb:=DM1.CrgDescrip('COD_PAR=' + QuotedStr('01'), 'CRE_PAR_GEN_SIS', 'URL_SERVICE_GENERAL');
     sServidorWeb:=sServidorWeb+'/reniec/get_DatosReniecLocal';
     Json:='{'
            +  '"ASODNI":"'+xAsodni+'"'
            +'}';
     try
       ...
       try
         ...
         begin
           Screen.Cursor := crDefault;
           MessageDlg('NO ES POSIBLE CONECTAR CON EL SERVICIO DE GENERALES - CONSULTA LOCAL RENIEC!', mtError,[mbOk],0);
           Result := NIL;
         end;
       end;
     finally
...
Function TDM1.EnviarSMS(xCelular, xMensaje: String): String;
Var xServerSMS:String;
  LHTTP: TIdHTTP;
  Params: TStringList;
  response:String;
Begin
   Try
      xServerSMS:=DM1.CrgDescrip('COD_PAR=' + QuotedStr('01'), 'CRE_PAR_GEN_SIS', 'URL_SER_SMS_REST');
      xMensaje:=copy(xMensaje,0,154);
      LHTTP := TIdHTTP.Create(Self);
      LHTTP.Request.ContentEncoding := 'UTF-8';
      Params := TStringList.Create;
      Try
         Try
            Params.Add('celular='+xCelular);
            Params.Add('mensaje='+xMensaje);
            response := LHTTP.Post(xServerSMS, Params);
         Finally
            Params.Free;
         End;
      Finally
      ...
      End;
   Except
      MessageDlg('HAY PROBLEMAS AL CONECTARSE CON EL SERVIDOR DE SMS', mtWarning, [mbOk], 0);
   End;
   result:= response;
end;
