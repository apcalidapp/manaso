...
uses ASODM,  ASO938, ASO936,  ASO944, ASO948, ASO900,   //RMZ ASO937,  ASO942, ASO943,
   ASO957, ASO260, ASO937, ASO943;
...
var xAsoid,xCodReg,xSQL,xselect:String;
	Boton,xItem:Integer;
begin
...
If Boton = ID_YES Then
   Begin
     ...
     DM1.cdsAsoX.Close;
     DM1.cdsAsoX.DataRequest('SELECT '+xSelect+' FROM APO201 WHERE ASOID='+QuotedStr(Trim(xAsoId)));
     DM1.cdsAsoX.Open;
     DM1.cdsAsoX.Edit;
     ...
     DM1.cdsAso.Close;
     DM1.cdsAso.DataRequest('SELECT '+xselect+' FROM APO201 WHERE ASOID='+QuotedStr(Trim(xAsoId)));
     DM1.cdsAso.Open;
     lkcDpto2.Text := DM1.cdsAso.fieldbyname('ASODPTLABID').AsString;
     lkcProv2.Text := DM1.cdsAso.fieldbyname('ASOPRVLABID').AsString;
     lkcDist2.Text := DM1.cdsAso.fieldbyname('ASODSTLABID').AsString;
...
//Fin MAS_202006_HPC
   xAsoId:=DM1.cdsAso.fieldbyname('ASOID').AsString;
   DM1.cdsAso.Close;  DM1.cdsAso.DataRequest('SELECT '+xSelect+' FROM APO201 WHERE ASOID='+QuotedStr(Trim(xAsoId)));  DM1.cdsAso.Open;
   If DM1.xCnd<>'M' Then DM1.CrgDatos;
   lkcDpto2.Text := DM1.cdsAso.fieldbyname('ASODPTLABID').AsString;
   lkcProv2.Text := DM1.cdsAso.fieldbyname('ASOPRVLABID').AsString;
   lkcDist2.Text := DM1.cdsAso.fieldbyname('ASODSTLABID').AsString;
...
begin
  ...
  XSQL := 'SELECT asoid, fecval, usuval, equusu, ofiusu, campo, feccad, datval, objeto, status, usudesval, fecdesval, obsdesval '
         +'FROM VAL_DAT_ASO WHERE ASOID='''+DM1.cdsAso.FieldByName('ASOID').AsString+''' order by fecval desc ';
