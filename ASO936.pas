...
// MAS_202309_HPC : Se controla que al momento de modificar el código modular este exista en la tabla de nuevos asociados, y igual que en la maestra de asociados.
unit ASO936;
...
type
  TfNueAsociado = class(TForm)
    ...
    pnlusuquemodifico: TPanel;
    lblusuquemodifico: TLabel;
    pnludesusuquemodifico: TPanel;
    meusumodreasigna: TMaskEdit;
...
begin
  ...
  // Se valida que codigo modular no sea repetio.
  If Not DM1.Validacodmodrepetido(EdtCodMod.Text, EdtAsoId.Text) Then Exit;
  // Fin: MAS_202204_HPC  
  ...
  Begin
    ...
    begin
      xSQL:='SELECT USRVERIFICA, FHVERIFICA FROM ASO003 WHERE ASOID = '+QuotedStr(Trim(DM1.cdsAso.fieldbyname('ASOID').AsString))
      +' AND IDESTADO = ''02'' AND NVL(REASIGNACION,''N'')= ''S'' ORDER BY FHVERIFICA DESC';
      DM1.cdsQry.Close; DM1.cdsQry.DataRequest(xSQL); DM1.cdsQry.Open;
      If DM1.cdsQry.RecordCount > 0 Then
      begin
        ...
        meusumodreasigna.Text:=DM1.cdsQry.FieldByName('USRVERIFICA').AsString;
      end
      else
      begin
        pnlFecha.Visible := True;
        ...
        meusumodreasigna.Text := 'No registra';
