...
begin
  ...
  xSql := 'SELECT * FROM'
  +'('
  +'SELECT ANO_EXP||NUM_EXP NUM_EXP, COD_MOD, LIB_ELE, NOM_GEN  FROM DB2ADMIN.MCABBEN0_1991'
  +' UNION'
  +' SELECT ANO||NUM_EXP NUM_EXP, COD_MOD, LIB_ELE, NOM_GEN  FROM DB2ADMIN.MCABBEN0_2000'
  +')'
  +' WHERE NOM_GEN LIKE '+QuotedStr('%'+Trim(xnombres)+'%');  
  DM1.cdsMGrupo.Close;
